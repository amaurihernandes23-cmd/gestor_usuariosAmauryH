...
def registrar_empleado():
    ...
    (nombre, documento, apellido, cargo, salariobru, horas_extra, bonificacion, salud, pension, salarioneto, departamento) 
...
def editar_empleado(id):
    ...
    if request.method == "POST":
        nombre = request.form["nombre"]
        apellido = request.form["apellido"]
        cargo = request.form["departamento"]
        documento = request.form["documento"]

        sql = """UPDATE empleado 
                 SET nombre=%s, apellido=%s, departamento=%s, documento=%s
                 WHERE id=%s"""

        cursor.execute(sql, (nombre, apellido, cargo, documento, id))
        con.commit()
